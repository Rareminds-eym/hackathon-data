...
export default function ExportSection({ projectCount, onExport }: ExportSectionProps) {
  ...
  return (
    <div className="relative">
      <div className="absolute inset-0 bg-gradient-to-br from-orange-500/5 to-pink-500/5 rounded-2xl md:rounded-3xl blur-xl"></div>
      ...
      <div className="relative backdrop-blur-sm bg-white/80 dark:bg-gray-900/80 border border-white/20 rounded-2xl md:rounded-3xl p-4 md:p-8 shadow-2xl">
        ...
        <div className="flex items-center gap-3 mb-4 md:mb-8">
          <div className="p-2 md:p-3 bg-gradient-to-r from-orange-500 to-pink-600 rounded-xl md:rounded-2xl shadow-lg">
            <FileText className="text-white" size={20} />
          </div>
          <div>
            <h2 className="text-lg md:text-2xl font-bold text-gray-900 dark:text-white">Export Tables</h2>
            <p className="text-xs md:text-sm text-gray-600 dark:text-gray-300">Download your data as CSV files</p>
          </div>
        </div>
        ...
        <div className="mb-4 md:mb-8">
          <div className="flex items-center gap-2 mb-3 md:mb-4">
            ...
            <h3 className="font-semibold text-gray-700 dark:text-gray-200 text-sm md:text-base">Tables to Export:</h3>
          </div>
          <div className="grid grid-cols-1 gap-2 md:gap-3">
            {requiredTables.map((table, index) => (
              <div key={table} className="group flex items-center gap-3 p-3 md:p-4 bg-gradient-to-r from-white/50 to-white/30 dark:from-gray-800/50 dark:to-gray-800/30 border border-white/20 rounded-lg md:rounded-xl backdrop-blur-sm transition-all duration-200 hover:scale-105 hover:shadow-lg">
                <div className="flex items-center justify-center w-6 h-6 md:w-8 md:h-8 bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg shadow-md">
                ...
                <div className="w-2 h-2 md:w-3 md:h-3 bg-gradient-to-r from-green-400 to-blue-500 rounded-full animate-pulse group-hover:animate-bounce"></div>
              </div>
            ))}
          </div>
        </div>
        ...
        {projectCount === 0 ? (
          <div className="flex items-start gap-3 p-4 md:p-6 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/20 rounded-xl md:rounded-2xl mb-4 md:mb-6 backdrop-blur-sm animate-in slide-in-from-top-2 duration-300">
            ...
            <div>
              ...
              <p className="text-yellow-600 dark:text-yellow-400 text-xs md:text-sm">Add at least one project to enable export functionality.</p>
            </div>
          </div>
        ) : (
          <div className="flex items-start gap-3 p-4 md:p-6 bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20 rounded-xl md:rounded-2xl mb-4 md:mb-6 backdrop-blur-sm animate-in slide-in-from-top-2 duration-300">
            <div className="p-2 bg-green-500/20 rounded-lg">
              <CheckCircle className="text-green-500 flex-shrink-0" size={20} />
            </div>
            <div>
              <p className="text-green-700 dark:text-green-300 font-semibold mb-1 text-sm md:text-base">Ready to Export</p>
          ...
          <div className="relative z-10 flex items-center gap-2 md:gap-3">
            {isExporting ? (
              <>
                ...
                <span className="text-sm md:text-lg">Exporting Tables...</span>
              </>
            ) : (
              <>
                ...
                <span className="text-sm md:text-lg">Export All Tables as ZIP</span>
              </>
            )}
          </div>
        </button>
        ...
        <div className="mt-4 md:mt-6 p-3 md:p-4 bg-gradient-to-r from-gray-500/10 to-slate-500/10 border border-gray-500/20 rounded-lg md:rounded-xl backdrop-blur-sm text-center">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <span className="text-xs md:text-sm font-medium text-gray-600 dark:text-gray-300">Export Information</span>
          </div>
